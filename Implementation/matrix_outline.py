...
def solution(rows, columns, queries):
    ...
    for query in queries:
        ...
        for i in range(y1, y2 + 1):
            square_list.append((x1, i))
        #오른쪽 높이 
        for i in range(x1 + 1, x2 + 1):
            square_list.append((i, y2))
        #아랫가로
        for i in range(y2 - 1, y1 - 1, -1):
            square_list.append((x2, i))
        #왼쪽 높이 
        ...
        #5. 넣어주기 -> import copy deecopy 도 가능  
        new_graph = [row[:] for row in graph]
        for (r, c) in square_list:
            n_r, n_c = rotation.popleft()
            new_graph[n_r][n_c] = graph[r][c]
        graph = new_graph
    return min_list
